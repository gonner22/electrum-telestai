...
class BTXPro(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.btxpro.com', '/v1.1/public/getticker?market=btc-tls')
        if ccy != "BTC":
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=TLS')
        return {ccy: to_decimal(rate) for (ccy, rate) in json["data"]["rates"].items()}

class CoinCap(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/assets/telestai/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/telestai/history?interval=d1&limit=2000')
        return dict([(timestamp_to_datetime(h['time']/1000, utc=True).strftime('%Y-%m-%d'), str(h['priceUsd']))
                     for h in history['data']])

class CoinEx(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.coinex.com', '/v1/market/ticker?market=tlsbtc')
        if ccy != "BTC":
            json2 = await self.get_json('api.coingecko.com', '/api/v3/simple/price?ids=bitcoin&vs_currencies=%s' % ccy)
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com',
                                   '/api/v3/simple/price?ids=telestai&vs_currencies=%s' % ccy)
        return {ccy: to_decimal(json['telestai'][ccy.lower()])}
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coingecko.com',
                                      f"/api/v3/coins/telestai/market_chart?vs_currency={ccy}&days={num_days}")
...
class CryptoCompare(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        result = await self.get_json('min-api.cryptocompare.com', '/data/price?fsym=TLS&tsyms={}&extraParams=ElectrumTLS'.format(tsyms))
    ...
    async def request_history(self, ccy):
        result = await self.get_json('min-api.cryptocompare.com', '/data/histoday?fsym=TLS&tsym={}&limit=100&aggregate=1&extraParams=ElectrumTLS'.format(ccy))
...
class Upbit(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.upbit.com', '/v1/ticker?markets=BTC-TLS')
        if ccy != "BTC":
            json2 = await self.get_json('api.coingecko.com', '/api/v3/simple/price?ids=bitcoin&vs_currencies=%s' % ccy)
