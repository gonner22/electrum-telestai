...
from electrum_tls.bitcoin import DummyAddress
from electrum_tls.plugin import run_hook
from electrum_tls.util import NotEnoughFunds, NoDynamicFeeEstimates, parse_max_spend
from electrum_tls.invoices import PR_PAID, Invoice, PR_BROADCASTING, PR_BROADCAST
from electrum_tls.transaction import Transaction, PartialTxInput, PartialTxOutput
from electrum_tls.network import TxBroadcastError, BestEffortRequestFailed
from electrum_tls.payment_identifier import (PaymentIdentifierType, PaymentIdentifier, invoice_from_payment_identifier,
    ...
    def check_onchain_outputs_and_show_errors(self, outputs: List[PartialTxOutput]) -> bool:
        ...
        for o in outputs:
            if o.scriptpubkey is None:
                self.show_error(_('Telestai Address is None'))
                return True
            if o.value is None:
