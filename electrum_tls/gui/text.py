...
try:
    ...
    from electrum_tls._vendor import pyperclip

import electrum_tls
from electrum_tls.gui import BaseElectrumGui
from electrum_tls.bip21 import parse_bip21_URI
from electrum_tls.util import format_satoshis, format_time
from electrum_tls.util import EventListener, event_listener
from electrum_tls.bitcoin import is_address, address_to_script, COIN
from electrum_tls.transaction import PartialTxOutput
from electrum_tls.wallet import Wallet, Abstract_Wallet
from electrum_tls.wallet_db import WalletDB
from electrum_tls.storage import WalletStorage
from electrum_tls.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from electrum_tls.interface import ServerAddr
from electrum_tls.invoices import Invoice
...
class ElectrumGui(BaseElectrumGui, EventListener):
    ...
    def __init__(self, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrum-tls create'")
            exit()
    ...
    def read_invoice(self):
        if self.lightning_invoice:
            ...
            self.show_message(_('Invalid Telestai address'))
            return
        return invoice
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy') or out.get('proxy user') or out.get('proxy pass'):
                new_proxy_config = electrum_tls.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
                if new_proxy_config:
