...
import electrum_tls
from electrum_tls import version, constants
from electrum_tls.i18n import _
from electrum_tls.logging import Logger, get_logger
from electrum_tls.bip21 import BITCOIN_BIP21_URI_SCHEME, LIGHTNING_URI_SCHEME
from electrum_tls.base_crash_reporter import BaseCrashReporter, EarlyExceptionsQueue
from electrum_tls.network import Network
from electrum_tls.plugin import run_hook
    ...
    def doNotify(self, wallet_name, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../icons/electrum.png')
            notification.notify('Electrum-TLS', message, app_icon=icon, app_name='Electrum-TLS')
        except ImportError:
...
class ElectrumQmlApplication(QGuiApplication):
    ...
    def __init__(self, args, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        not_loaded = QFontDatabase.addApplicationFont('electrum_tls/gui/qml/fonts/PTMono-Regular.ttf') < 0
        not_loaded = QFontDatabase.addApplicationFont('electrum_tls/gui/qml/fonts/PTMono-Bold.ttf') < 0 and not_loaded
        if not_loaded:
        ...
        self.context.setContextProperty('UI_UNIT_NAME', {
            "FEERATE_SAT_PER_VBYTE": electrum_tls.util.UI_UNIT_NAME_FEERATE_SAT_PER_VBYTE,
            "FEERATE_SAT_PER_VB":    electrum_tls.util.UI_UNIT_NAME_FEERATE_SAT_PER_VB,
            "TXSIZE_VBYTES":         electrum_tls.util.UI_UNIT_NAME_TXSIZE_VBYTES,
            "MEMPOOL_MB":            electrum_tls.util.UI_UNIT_NAME_MEMPOOL_MB,
        })
