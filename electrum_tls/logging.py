...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_tls."):
...
def _delete_old_logs(path, *, num_files_keep: int):
    files = sorted(list(pathlib.Path(path).glob("electrum_tls_log_*.log")), reverse=True)
...
def _configure_file_logging(log_directory: pathlib.Path, *, num_files_keep: int):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_tls_log_{timestamp}_{PID}.log"
...
#       as possible.
# Note: some users might use Electrum-TLS as a python library and not use run_electrum_tls,
...
electrum_logger = logging.getLogger("electrum_tls")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_tls."):
        name = name[9:]
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    _logger.info(f"Electrum-TLS version: {ELECTRUM_VERSION} - https://telestai.io - {GIT_REPO_URL}")
