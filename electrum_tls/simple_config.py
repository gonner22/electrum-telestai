...
class SimpleConfig(Logger):
    ...
    def electrum_path(self):
        ...
        self.logger.info(f"electrum-tls directory {path}")
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'tls':8, 'mtls':5, 'utls':2, 'sats':2, 'sat':0}
    ...
    def cv(config):
        ...
        'electrum-test1.telestai.io:51002:s'
    ...
    def _default_swapserver_url(self) -> str:
        if constants.net == constants.BitcoinMainnet:
            default = 'https://swaps.telestai.io/api'
        elif constants.net == constants.BitcoinTestnet:
            default = 'https://testnet-swaps.telestai.io/api'
        else:
...
Warning: setting this to too low will result in lots of payment failures."""),
    ...
    BLOCK_EXPLORER = ConfigVar(
        'block_explorer', default='explorer.telestai.io', type_=str,
...
def read_user_config(path: Optional[str]) -> Dict[str, Any]:
    """Parse and store the user config settings in electrum-tls.conf into user_config[]."""
    if not path:
        return {}
