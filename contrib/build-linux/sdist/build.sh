...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-tls-sdist-builder-img \
    "$CONTRIB_SDIST"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum_build/sdist/fresh_clone/electrum-tls"
...
docker run -it \
    --name electrum-tls-sdist-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum-tls \
    --rm \
    --workdir /opt/electrum-tls/contrib/build-linux/sdist \
    --env OMIT_UNCLEAN_FILES \
    electrum-tls-sdist-builder-img \
    ./make_sdist.sh
