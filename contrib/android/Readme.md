...
The Qml GUI is used with Electrum-TLS on Android devices, since Electrum-TLS 4.4.
To generate an APK file, follow these instructions.

(note: older versions of Electrum-TLS for Android used the "kivy" GUI)
...
```
$ adb -d install -r dist/ElectrumTLS-*-arm64-v8a-debug.apk
$ adb shell monkey -p org.it4f.electrumtls 1
```
...
```
$ docker run -it --rm \
    -v $PWD:/home/user/wspace/electrum-tls \
    -v $PWD/.buildozer/.gradle:/home/user/.gradle \
    --workdir /home/user/wspace/electrum-tls \
    electrum-tls-android-builder-img
```
...
```
adb logcat | grep -F "`adb shell ps | grep org.it4f.Electrumtls | cut -c14-19`"
```
...
Run electrum-tls with the `-g` switch: `electrum_tls -g qml`
...
```
$ adb shell
adb$ run-as org.it4f.electrumtls ls /data/data/org.it4f.electrumtls/files/data
adb$ exit
$ adb exec-out run-as org.it4f.electrumtls cat /data/data/org.it4f.electrumtls/files/data/wallets/my_wallet > my_wallet
```
To push a file:
```
...
adb$ run-as org.it4f.testnet.electrumtls cp /data/local/tmp/my_wallet /data/data/org.it4f.testnet.electrumtls/files/data/testnet/wallets/
