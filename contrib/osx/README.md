...
This guide explains how to build Electrum-TLS binaries for macOS systems.
...
- The unix username should be `vagrant`, and `electrum-tls` should be cloned directly
  to the user's home dir: `/Users/vagrant/electrum-tls`.
...
- Installing extraneous brew packages can result in build differences.
  ...
  So having a dedicated "electrum-tls binary builder macOS VM" is recommended.
- Make sure that you are building from a fresh clone of electrum-tls
...
#### 2. Build Electrum-TLS

    cd electrum-tls
    ./contrib/osx/make_osx.sh

This creates both a folder named Electrum-TLS.app and the .dmg file (both unsigned).
...
also run the `sign_osx.sh` script:
    ...
    CODESIGN_CERT="Developer ID Application: Telestai Developers (A82F56PX3Q)" \
...
2. Use the provided `compare_dmg` script to compare the binary you built with
   the official release binary.
    ```
    $ ./contrib/osx/compare_dmg dist/electrum-tls-*.dmg electrum_dmg_official_release.dmg
    ```
