...
PACKAGE=Electrum-TLS
GIT_REPO=https://github.com/telestai/electrum-tls
...
cp -f "$DLL_TARGET_DIR"/libsecp256k1.*.dylib "$PROJECT_ROOT/electrum_tls/" || fail "Could not copy libsecp256k1 dylib"
...
cp -f "$DLL_TARGET_DIR/libzbar.0.dylib" "$PROJECT_ROOT/electrum_tls/" || fail "Could not copy ZBar dylib"
...
cp -f "$DLL_TARGET_DIR/libusb-1.0.dylib" "$PROJECT_ROOT/electrum_tls/" || fail "Could not copy libusb dylib"
...
cp "$PROJECT_ROOT/electrum_tls"/libsecp256k1.*.dylib "$VENV_DIR/lib/python$PY_VER_MAJOR/site-packages/electrum_tls/"
...
VERSION=$(python3 -c "import electrum_tls; print(electrum_tls.version.ELECTRUM_VERSION)")
...
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-tls-$VERSION-unsigned.dmg || fail "Could not create .DMG"
